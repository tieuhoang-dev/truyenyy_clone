...
const StoryDetailContent = () => {
    ...
    useEffect(() => {
        ...
        const fetchStory = async () => {
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/stories/${storyID}`);
        ...
        const fetchChapters = async () => {
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/stories/${storyID}/chapters`);
                const data = await res.json();

                if (data && Array.isArray(data.chapters)) {
                    const sortedChapters = data.chapters
                        .map((chapter: { id: string; title: string; chapter_number: number; updated_at?: string }) => ({
                            _id: chapter.id,
                            title: chapter.title,
                            number: chapter.chapter_number,
                            updatedAt: chapter.updated_at,
                        }))
                        .sort((a: { updatedAt?: string }, b: { updatedAt?: string }) => new Date(b.updatedAt || 0).getTime() - new Date(a.updatedAt || 0).getTime());

                    const latestChapters = sortedChapters.slice(0, 3);
